...
public:
	...
	MC_Str &operator=(const MC_Str &aString)
	{
		if (this != &aString)
			this->operator=(aString.GetBuffer());
		return *this;
	}
	...
	friend MC_Str operator+(const MC_Str &aString, const C *aString2)
	{
		MC_Str s = aString;
		s += aString2;
		return s;
	}
	friend MC_Str operator+(const MC_Str &aString, const C aChar)
	{
		MC_Str s = aString;
		s += aChar;
		return s;
...
protected:
	...
	static void InternalSetBuffer(MC_Str<C, S> &aStr, C *aPointer) { assert(0 && "not allowed on static strings!"); }
	template <class C, int S>
	static C *InternalGetBuffer(MC_Str<C, S> &aStr) { return aStr.myBuffer; }
	template <class C, int S>
	static const C *InternalGetBuffer(const MC_Str<C, S> &aStr) { return aStr.myBuffer; }
	template <class C, int S>
	static int InternalGetBufferSize(const MC_Str<C, S> &aStr) { return S; }
	template <class C, int S>
	static void InternalSetBufferSize(MC_Str<C, S> &aStr, int aSize) { assert(0 && "not allowed on static strings!"); }
	template <class C, int S>
	static void InternalReserve(MC_Str<C, S> &aStr, int aLength) { assert(aLength < S); }

	// Specialization for dynamic string
	template <class C>
	static void InternalSetBuffer(MC_Str<C, 0> &aStr, C *aPointer) { aStr.myDynamicPointer = aPointer; }
	template <class C>
	static C *InternalGetBuffer(MC_Str<C, 0> &aStr) { return aStr.myDynamicPointer; }
	template <class C>
	static const C *InternalGetBuffer(const MC_Str<C, 0> &aStr) { return aStr.myDynamicPointer; }
	template <class C>
	static int InternalGetBufferSize(const MC_Str<C, 0> &aStr) { return (MC_STRING_IS_NOT_NULL(aStr.myDynamicPointer)) ? *(int *)(aStr.myDynamicPointer - sizeof(int) / sizeof(C)) : 0; }
	template <class C>
	static void InternalSetBufferSize(MC_Str<C, 0> &aStr, int aSize)
	{
		if (MC_STRING_IS_NOT_NULL(aStr.myDynamicPointer))
			*(int *)(aStr.myDynamicPointer - sizeof(int) / sizeof(C)) = aSize;
	}
	template <class C>
	static void InternalReserve(MC_Str<C, 0> &aStr, int aLength)
	{
		if (InternalGetBufferSize(aStr) <= aLength)
			aStr.ReallocData(aLength);
	}

	// Conversion functions
	template <class C>
	C InternalConvertChar(TCHAR aChar) const;
	template <class C>
	C InternalConvertChar(wchar_t aChar) const;

	template <>
	TCHAR InternalConvertChar<TCHAR>(TCHAR aChar) const { return aChar; }
	template <>
	TCHAR InternalConvertChar<TCHAR>(wchar_t aChar) const { return TCHAR(aChar); }

	template <>
	wchar_t InternalConvertChar<wchar_t>(TCHAR aChar) const { return wchar_t(aChar); }
	template <>
	wchar_t InternalConvertChar<wchar_t>(wchar_t aChar) const { return aChar; }

	// 8-bit (TCHAR) functions
	static void InternalMakeUpper(TCHAR *aPointer, int aBufferSize)
	{
		if (aPointer && aBufferSize)
			_tcsupr_s(aPointer, aBufferSize);
	}
	static void InternalMakeLower(TCHAR *aPointer, int aBufferSize)
	{
		if (aPointer && aBufferSize)
			_tcslwr_s(aPointer, aBufferSize);
	}
	...
	static bool InternalEndsWith(const TCHAR *aString1, const TCHAR *aString2)
	{
		size_t len1 = _tcslen(aString1);
		size_t len2 = _tcslen(aString2);
		return len1 >= len2 ? (_tcscmp(aString1 + len1 - len2, aString2) == 0) : false;
	}
	static bool InternalEndsWithNoCase(const TCHAR *aString1, const TCHAR *aString2)
	{
		size_t len1 = _tcslen(aString1);
		size_t len2 = _tcslen(aString2);
		return len1 >= len2 ? (_tcsicmp(aString1 + len1 - len2, aString2) == 0) : false;
	}
	static bool InternalBeginsWith(const TCHAR *aString1, const TCHAR *aString2)
	{
		size_t len1 = _tcslen(aString1);
		size_t len2 = _tcslen(aString2);
		return len1 >= len2 ? (_tcsncmp(aString1, aString2, len2) == 0) : false;
	}
	static bool InternalBeginsWithNoCase(const TCHAR *aString1, const TCHAR *aString2)
	{
		size_t len1 = _tcslen(aString1);
		size_t len2 = _tcslen(aString2);
		return len1 >= len2 ? (_tcsncicmp(aString1, aString2, len2) == 0) : false;
	}
	...
	static void InternalMakeUpper(wchar_t *aPointer, int aBufferSize)
	{
		if (aPointer && aBufferSize)
			MC_MakeUpper(aPointer, aBufferSize);
	}
	static void InternalMakeLower(wchar_t *aPointer, int aBufferSize)
	{
		if (aPointer && aBufferSize)
			MC_MakeLower(aPointer, aBufferSize);
	}
	...
	static bool InternalEndsWith(const wchar_t *aString1, const wchar_t *aString2)
	{
		size_t len1 = wcslen(aString1);
		size_t len2 = wcslen(aString2);
		return len1 >= len2 ? (wcscmp(aString1 + len1 - len2, aString2) == 0) : false;
	}
	static bool InternalEndsWithNoCase(const wchar_t *aString1, const wchar_t *aString2)
	{
		size_t len1 = wcslen(aString1);
		size_t len2 = wcslen(aString2);
		return len1 >= len2 ? (wcsicmp(aString1 + len1 - len2, aString2) == 0) : false;
	}
	static bool InternalBeginsWith(const wchar_t *aString1, const wchar_t *aString2)
	{
		size_t len1 = wcslen(aString1);
		size_t len2 = wcslen(aString2);
		return len1 >= len2 ? (wcsncmp(aString1, aString2, len2) == 0) : false;
	}
	static bool InternalBeginsWithNoCase(const wchar_t *aString1, const wchar_t *aString2)
	{
		size_t len1 = wcslen(aString1);
		size_t len2 = wcslen(aString2);
		return len1 >= len2 ? (wcsnicmp(aString1, aString2, len2) == 0) : false;
	}
...
public:
	...
	MC_StaticString &operator=(const MC_StaticString &aString)
	{
		if (this != &aString)
			this->operator=(aString.GetBuffer());
		return *this;
	}
	MC_StaticString &operator=(const TCHAR *aString)
	{
		MC_Str<TCHAR, S>::operator=(aString);
		return *this;
	}
	MC_StaticString &operator=(const wchar_t *aString)
	{
		MC_Str<TCHAR, S>::operator=(aString);
		return *this;
	}
	MC_StaticString &operator=(TCHAR aChar)
	{
		MC_Str<TCHAR, S>::operator=(aChar);
		return *this;
	}
	MC_StaticString &operator=(wchar_t aChar)
	{
		MC_Str<TCHAR, S>::operator=(aChar);
		return *this;
	}
...
public:
	...
	MC_StaticLocString &operator=(const MC_StaticLocString &aString)
	{
		if (this != &aString)
			this->operator=(aString.GetBuffer());
		return *this;
	}
	MC_StaticLocString &operator=(const TCHAR *aString)
	{
		MC_Str<wchar_t, S>::operator=(aString);
		return *this;
	}
	MC_StaticLocString &operator=(const wchar_t *aString)
	{
		MC_Str<wchar_t, S>::operator=(aString);
		return *this;
	}
	MC_StaticLocString &operator=(TCHAR aChar)
	{
		MC_Str<wchar_t, S>::operator=(aChar);
		return *this;
	}
	MC_StaticLocString &operator=(wchar_t aChar)
	{
		MC_Str<wchar_t, S>::operator=(aChar);
		return *this;
	}
...
{
public:
	MC_Str<C, S> &Delete(int nIndex, int nCount = 1); // Default argument in declaration
																										// Other members...
};

template <class C, int S>
MC_Str<C, S> &MC_Str<C, S>::Delete(int nIndex, int nCount) // No default argument here
{
	if (nIndex < 0)
		return *this;
