...
#include <windows.h> // For InterlockedExchange and LONG_PTR
...
__declspec(align(64)) struct ListenerHandle
{
...
// #ifndef	_RELEASE_
	...
	for (int i = 0; i < locNumDebugListeners; i++)
	{
		...
		{
			_InterlockedExchangePointer((volatile PVOID *)&handle.listener, (PVOID)aListener); // handle.listener = aListener;
	...
	_InterlockedExchangePointer((volatile PVOID *)&locOurDebugListeners[newIndex].listener, (PVOID)aListener);
...
bool MC_Debug::AddErrorListener(MC_DebugListener *aListener)
{
	...
	assert(newIndex < MAXLISTENERS);
	_InterlockedExchangePointer((volatile PVOID *)&locOurErrorListeners[newIndex].listener, (PVOID)aListener);
	...
	case VER_PLATFORM_WIN32_NT:
		...
		if (osvi.dwMajorVersion == 6 && osvi.dwMinorVersion == 0)
		{
			if (osvi.wProductType == VER_NT_WORKSTATION)
				...
				strcat_s(osDescription, sizeof(osDescription), "Windows Server \"Longhorn\" ");
		}
		...
		if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 2)
		{
							 ...
							 si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64)
			{
				...
				strcat_s(osDescription, sizeof(osDescription), "Windows Server 2003, ");
		}
		...
		if (bOsVersionInfoEx)
		{
					...
					si.wProcessorArchitecture != PROCESSOR_ARCHITECTURE_AMD64)
			{
				if (osvi.dwMajorVersion == 4)
					...
					strcat_s(osDescription, sizeof(osDescription), "Professional ");
			}
							 ...
							 osvi.wProductType == VER_NT_DOMAIN_CONTROLLER)
			{
				if (osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 2)
				{
					...
					else if (si.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64)
					{
						if (osvi.wSuiteMask & VER_SUITE_DATACENTER)
							...
							strcat_s(osDescription, sizeof(osDescription), "Standard x64 Edition ");
					}
					...
					else
					{
						if (osvi.wSuiteMask & VER_SUITE_DATACENTER)
							...
							strcat_s(osDescription, sizeof(osDescription), "Standard Edition ");
					}
				}
				...
				else // Windows NT 4.0
				{
					if (osvi.wSuiteMask & VER_SUITE_ENTERPRISE)
						...
						strcat_s(osDescription, sizeof(osDescription), "Server 4.0 ");
				}
			}
		}
...
const char *MC_Debug::GetSystemInfoString()
{
	...
	{
		aString += temp.Format("Process Affinity Mask:    0x%08llx\n", processAffinityMask);
		aString += temp.Format("System Affinity Mask:     0x%08llx\n", systemAffinityMask);
	}
	else
	{
...
MC_Debug::InternalPosTracer::InternalPosTracer(TraceType aType, const char *aFile, const char *aFunction, unsigned int aLine)
		: myType(aType), myFile(aFile), myFunction(aFunction), myLine(aLine)
{
}
